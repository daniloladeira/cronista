...
>
> **Validado contra Ollama real em 2026-08-22** (`llama3.1:8b`, primeiro candidato — RTX 4060, 6,9 GB livres, folga confortável pro tamanho do modelo). Achado real, não presumido: a reunião do CT-16/36 (fala real + YouTube misturado) foi **recusada pelo modelo** — os filtros de segurança do Llama trataram o conteúdo informal como controverso. Validado então com uma transcrição sintética (reunião fictícia, tom normal): pipeline funcionou de ponta a ponta, mas achou uma lacuna real de qualidade — o modelo não ligava "eu fico responsável" ao falante daquele trecho, e classificava pendência como decisão, deixando "Pendências" vazio quando havia ação combinada de verdade. Prompt ajustado (`PROMPT_VERSION` v1 → v2, instrução explícita sobre primeira pessoa e a diferença Decisão/Pendência): melhorou parcialmente (passou a reconhecer "eu" como o falante) mas **não corrigiu Pendências continuar vazio** — limitação de capacidade do modelo em 8B parâmetros, não do prompt. Registrado como achado real, decisão de seguir ou trocar de modelo em aberto.
