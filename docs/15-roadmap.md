...
**Não fecha o "pronto quando" ainda.** Três rodadas de prompt hoje (v1→v4) resolveram os problemas que apareceram um de cada vez, mas cada correção parece destravar um problema novo — típico de ajuste fino em modelo de 8B parâmetros perto do limite do que ele consegue fazer de forma confiável nessa tarefa. Op­ções em aberto: mais uma rodada de prompt, aceitar como limitação documentada, ou testar um modelo maior — decisão não tomada ainda.
