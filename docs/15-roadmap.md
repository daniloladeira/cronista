...
2. **Conteúdo: qualidade real ainda tem lacuna, não corrigida.** Com a formatação certa, "Decisões" saiu **vazio**, apesar da reunião ter decisões claras (não importar dados de agenda, não importar dados financeiros, priorizar simplicidade na migração em vez de replicar estrutura). E duas "Pendências"/"Pontos em aberto" saíram **invertidas** em relação ao que foi dito — o resumo sugere "importar dados financeiros" e "importar agendamentos futuros" quando a reunião decidiu o oposto dos dois. Isso não foi corrigido ainda; é o mesmo tipo de limitação de classificação que apareceu com dado sintético (docs/13 §4), agora confirmada com dado real e em escala que passa pelo caminho de blocos (RF-17) — o que sugere que a consolidação entre blocos pode estar perdendo ou invertendo contexto, não só a extração de um bloco isolado.

**Não fecha o "pronto quando" ainda.** As quatro seções aparecem (satisfaz metade do critério), mas a precisão do conteúdo — sobretudo Decisões saindo vazio e inversões factuais — não está no nível que "resumido bem" exige. Decisão de quanto investir em mais prompt, trocar de modelo, ou aceitar como limitação conhecida documentada, em aberto.
