...
**Não fecha o "pronto quando" ainda.** Quatro rodadas de prompt (v1→v4) e dois modelos testados hoje contra a mesma reunião real — cada ajuste resolve um problema e revela outro, e trocar de modelo (dentro da faixa que cabe no orçamento de VRAM desta máquina) trocou de defeito, não removeu defeito. Isso é evidência real de que o gargalo não é (só) escolha de modelo ou texto de prompt — pode ser a abordagem de bloco+consolidação em si, nessa faixa de tamanho de modelo (8-9B). Decisão de quanto investir mais aqui, ou aceitar como limitação documentada por ora, em aberto.
