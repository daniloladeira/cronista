...
**CT-36 medido provisoriamente em 2026-08-22, sobre os primeiros 20min dessa mesma reunião: WER 0,0%** (1755 palavras, 0 substituições/remoções/inserções), com `scripts/measure_wer.py`. **Ressalva de método, registrada por transparência:** a referência (`ct36/referencia_20min.txt`) partiu de uma cópia da própria saída do Whisper, revisada de ouvido — na primeira conferência, o usuário identificou de verdade três problemas (um loop de alucinação repetindo "Me agrada" várias vezes, "cheerleader" transcrito como "char leader", e uma repetição de "e" de origem incerta), mas essas correções não chegaram a ser salvas no arquivo de referência antes de aprovar seguir assim mesmo. Ou seja, o WER de 0% aqui **não é evidência de transcrição perfeita** — é o piso que se obtém quando a referência não diverge do que se está medindo. Não invalida CT-16/CT-17 nem o restante da Fase 3, mas não deve ser lido como a linha de base de WER real do projeto. Uma medição limpa (referência de fato corrigida, sobre reunião sem mistura de áudio externo) segue como pendência real.
